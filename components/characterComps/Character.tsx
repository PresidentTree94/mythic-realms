...
export default function Character({ data }: { data: { character: CharacterType, homeland: TerritoryType | null, residence: TerritoryType | null } }) {
  return (
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent flex flex-col justify-between p-6">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap font-semibold font-body text-xs uppercase tracking-tighter mb-2">
            ...
            {data.residence && data.homeland?.name !== data.residence.name && <span className="bg-secondary/80 text-[hsl(25,30%,15%)] px-2 py-1 backdrop-blur-sm">{data.residence.name}, {data.residence.kingdoms.name}</span>}
          </div>
